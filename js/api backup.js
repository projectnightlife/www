...
document.documentElement.onclick = function(event)
{
  ...
  {
	if (target.id == 'fblogin')
	  api.fbconnect();
	else
      api.sendSimpleRequest(target.href, target.parentNode);
	return false;
  }
  ...
  else if (target.type == 'submit' && target.parentNode.getAttribute('ajaxify'))
  {
	api.sendForm(target.parentNode);
	return false;
  }
};
